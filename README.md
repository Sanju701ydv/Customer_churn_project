...
Customer churn is a major cost driver for banks and subscription-based businesses. This project uses a neural network trained on historical customer data to predict the probability that a given customer will churn, based on features like credit score, age, balance, and account activity. The final model is wrapped in an interactive Streamlit app where users can input customer details and instantly get a churn prediction.
...
4. **Deployment**
   - Saved the trained model and preprocessing objects (encoders, scaler) using `pickle` / `model.save()`
